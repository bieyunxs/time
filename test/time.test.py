...
def update_time_info():
    ...
    if current_period['name'] != 'free time':
        if current_period['end']:
            period_end = datetime.combine(current_datetime.date(), current_period['end'])
            time_remaining = period_end - current_datetime
            if time_remaining.total_seconds() > 0:
                show_remaining_time(remaining_label, time_remaining)
            else:
                remaining_label.config(text="本时间段即将结束")
    else:
        # 计算到下一个时间段的开始时间
        next_period_start = min((x for x in time_periods if x['start'] > current_time),
                                key=lambda x: (datetime.combine(date.today(), x['start']) - current_datetime).total_seconds(),
                                default={'start': time(0)})
        time_difference = datetime.combine(date.today(), next_period_start['start']) - current_datetime

        # 显示剩余时间或今天剩余时间
        if time_difference.days == 1 and time_difference.seconds < 12 * 3600:
            show_remaining_time(remaining_label, time_difference)
        elif time_difference.total_seconds() < 6 * 3600:
            show_remaining_time(remaining_label, time_difference)
        else:
            show_remaining_time_till_midnight(remaining_label, current_datetime)

    root.after(500, update_time_info)  # 每半秒更新一次时间信息

# 显示剩余时间
def show_remaining_time(label, time_difference):
    minutes, seconds = divmod(time_difference.seconds, 60)
    label.config(text=f"剩余时间: {minutes:02d}:{seconds:02d}")

# 显示到午夜的剩余时间
def show_remaining_time_till_midnight(label, current_datetime):
    end_of_day = datetime.combine(current_datetime.date(), time(23, 59, 59))
    time_difference_to_end_of_day = end_of_day - current_datetime
    remaining_hours, remaining_minutes, remaining_seconds = format_remaining_time(time_difference_to_end_of_day)
    label.config(text=f"今天还有{remaining_hours:02d}小时{remaining_minutes:02d}分钟{remaining_seconds:02d}秒")

# 格式化剩余时间
def format_remaining_time(time_difference):
    return time_difference.seconds // 3600, (time_difference.seconds // 60) % 60, time_difference.seconds % 60
...
# 创建主窗口
root = tk.Tk()
root.title("time2.0.first_ver.first_ver")
