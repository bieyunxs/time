...
class TimeManageApp:
    def __init__(self, root):
        self.root = root
        self.custom_period_name = tk.StringVar()
        self.custom_period_start = tk.StringVar()
        self.custom_period_end = tk.StringVar()

    def add_custom_period_popup(self):
        custom_period_popup = tk.Toplevel()
        custom_period_popup.title("time add")

        # 创建输入框和确认按钮
        period_name_entry = tk.Entry(custom_period_popup)
        period_name_label = tk.Label(custom_period_popup, text="时间段名称：")
        period_name_label.pack()
        period_name_entry.pack()

        period_start_time_entry = tk.Entry(custom_period_popup)
        period_start_time_label = tk.Label(custom_period_popup, text="开始时间（00:00）：")
        period_start_time_label.pack()
        period_start_time_entry.pack()

        period_end_time_entry = tk.Entry(custom_period_popup)
        period_end_time_label = tk.Label(custom_period_popup, text="结束时间（00:00）：")
        period_end_time_label.pack()
        period_end_time_entry.pack()

        def add_custom_period():
            period_name = period_name_entry.get()
            start_time = period_start_time_entry.get()
            end_time = period_end_time_entry.get()

            if period_name == "" or start_time == "" or end_time == "":
                tk.messagebox.showerror("错误", "请完整输入时间段信息")
            else:
                # 执行添加时间段的操作，并更新时间段信息的数据结构
                # 这里应该包含你添加时间段的逻辑

                tk.messagebox.showinfo("成功", "时间段添加成功")
                custom_period_popup.destroy()

        confirm_button = tk.Button(custom_period_popup, text="确认", command=add_custom_period)
        confirm_button.pack()
...
def open_settings():
    ...
    def save_settings():
        ...
        if always_on_top:
            messagebox.showinfo(parent=settings_win, title="设置已更新", message="窗口将始终保持在最上层。")
        else:
            messagebox.showinfo(parent=settings_win, title="设置已更新", message="窗口将不再始终保持在最上层。")
...
# 新增时间添加功能按钮
app = TimeManageApp(root)
time_add_button = tk.Button(root, text="时间段添加", command=app.add_custom_period_popup)
time_add_button.pack(side=tk.LEFT, padx=10, pady=10)  # 放置在左侧
